...
- **테스트**: **126 collected · 117 passed**(총평 14종 포함)(+9 live-LLM opt-in skip) — 파이프라인·사정·해설 validator·상담 36종·API. (티어1 정리 전 전체 스위트는 236이었음 — 현행은 졸업센터 전용 수치.) 결정론 3회 해시 동일.
...
- 해설 일부 줄은 "요람 원문에서 직접 확인되지 않음" 표시로 남음(전 섹션 인용률 S3 7/7 · S2 6/8 · S4 5/7) — 의도된 validator 동작이며 데모에선 "환각을 잡는 장면"으로 사용.
