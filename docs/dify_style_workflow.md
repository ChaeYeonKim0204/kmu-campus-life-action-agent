...
> 지식검색/LLM/IF·ELSE/반복/답변)로 번역한 것. 교수가 선호하는 Dify식 분기 그래프 표기.
> 화면 그래프(BASE 16 + 상담 7)와 토폴로지 동일하되, **Dify 표현을 위해 내부 단계 3개를 명시
> 노드로 분해**했다 — pre 필터(화면에선 '갈림길 선정' 노드 내부)·검토 원장(총평 카드의 '검토 후
> 제외' 토글)·미확인 표시(해설 줄 단위 표기). 과장 아님 주의. (mermaid는 Notion·GitHub·VSCode 렌더링)
...
flowchart TD
    ...
    IF2 -->|위반 줄| MARK["⚙ 코드: '요람 원문에서 직접<br/>확인되지 않음' 표시"] --> AGG
    ...
    IF4 -->|"위반 줄 폐기(전량 폐기 시 1회만 재시도)"| L3
