...
- **테스트**: **139 collected · 130 passed**(총평 17종 + 데모 학생 회귀 5종 포함)(+9 live-LLM opt-in skip) — 파이프라인·사정·해설 validator·상담 36종·API. (티어1 정리 전 전체 스위트는 236이었음 — 현행은 졸업센터 전용 수치.) 결정론 3회 해시 동일.
...
tests/test_v2_*.py   89 테스트 전부 passed(상담·총평·데모 학생 회귀 5 포함; 전체 스위트 139 collected·130 passed)
```
