...
5. **"이거 맞아?" 확인** — 폐강은 자동 제외, F·재수강한 과목은 내가 체크 → 데이터 확정.
...
| ⏳ **후순위 (나중에)** | 다중 경로 비교(대체과목 vs 계절학기 vs 조기졸업 A·B·C 시나리오 비교) |
| 🚫 **안 만듦 (오버스코프)** | OCR 성적증명서 경로 · ON국민 다운로드 안내 가이드(여력 시) · 엑셀 에러처리 · F/재수강 자동판별 · 전 학과/전 연도 |
...
- **컴퓨터(결정론) = 사실 담당**: 학점 계산, 요람 조회, 과목 매칭, 부족분 산출, 리스크 등급, 검증. → 틀릴 일 없음.
...
│ ① 현황   총학점 ▓▓▓▓▓▓▓▓░░ 108/130 · 전공필수 18/18 ✅ · 전공선택 24/30 ⚠️ 6 부족        │
...
**데모 킬러 장면**: 조건을 토글한다 — *"남은 학기 2→3"* 또는 *"이번 학기 5과목 초과 금지"* → **로드맵이 즉석에서 다시 짜짐**. 근데 옆 패널엔 **모든 과목·요건이 요람 몇 페이지 근거인지** 표시. → "생각도 하고 + 근거도 있다" = ChatGPT랑 눈에 보이게 다름.
