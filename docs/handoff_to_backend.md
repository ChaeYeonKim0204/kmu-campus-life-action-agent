...
│ tests/test_*             ← 작성 당시 회귀 PASS, 현재 HEAD 재검증 필요 │
...
> **모델 팀이 백엔드 영역에 예외로 손댄 곳은 제한적**입니다 (`tools/document_drafter.py`의 graduation_audit 흐름 일부, `agent/answer_validator.py`의 출력 개인정보 패턴 5개 추가, P4·b로 분리 commit). 모두 commit message에 사유 명시.
...
- `/actions/continue` 도 비슷함
  - privacy_blocked / needs_input / completed / output_privacy_blocked 분기마다 공통 필드가 다름
  - 특히 completed 응답도 현재 `sources`, `citations`, `tool_logs`를 반환하지 않음
...
- 매핑 예:
  ...
  - `fallback_used > 0` → "일부 공식 페이지는 최신 확인 대신 저장/대체 근거를 사용했습니다"
...
**작업:**
- `app.py` / `agent.action_state` / `tools.document_drafter.py` 경계에서 action grounding policy 확인
- `/actions/continue` 검색 query를 `action_id`만 쓰지 않고 action issue type, original query, selected source ids, action label까지 활용할 수 있게 요청/상태 흐름 보강
...
**손댈 파일:** `app.py`, `agent/action_state.py`, `tools/document_drafter.py`, `ActionContinueRequest` 모델
...
**현재:** ACTION_SCHEMAS 전반에서 `_optional` 이름인데 required_slots 안에 들어있는 항목이 있음 (예: `evidence_document_type_optional`, `instructor_name_optional`, `target_semester_optional` 등). 사용자가 "없음" / "모름" 등을 명시적으로 채워야 함.
...
#### **B6. `/health`의 `agent_metrics` aggregate 필드** (§20)
...
- `/health` 응답의 `agent_metrics` 필드는 파일 tail 1000줄 정도 읽고 카운트·비율 aggregate
...
| 🟡 3 | B6. agent_metrics aggregate | 운영 어필 (관찰 가능성) | 3h |
...
| Sanitize 패턴 dict | `graduation_center/service.py:SENSITIVE_PATTERNS` | B2 또는 향후 privacy validator 작업에서 동일 패턴 dict 재사용 가능 (이미 `answer_validator`엔 통일됨) |
...
`work_log_2026-05-28.md` 작성 당시에는 일반 회귀와 `tests/test_graduation_8_tasks.py`의 8개 live 회귀 PASS 기록이 있습니다. 다만 현재 HEAD 기준으로는 이후 `dd0bc77`, `f518f9e`가 추가되어 테스트 수집 숫자가 달라졌습니다.

현재 확인 기준:

```text
Python 3.11 환경 테스트 수집: 총 144개
일반 테스트: 135개
live_llm 테스트: 9개
```
...
- 기본 shell Python 3.8.5에서는 프로젝트 테스트 실행 환경이 맞지 않습니다.
- 현재 Python 3.11 환경에서도 FastAPI `TestClient` 기반 API 테스트가 멈추는 현상이 있어, 발표 전 dependency 버전 고정 후 전체 재검증이 필요합니다.
- 문서의 PASS 숫자는 "작성 당시 환경의 기록"으로 보고, 발표 자료에는 최신 재검증 숫자를 다시 찍어 반영하는 것을 권장합니다.
