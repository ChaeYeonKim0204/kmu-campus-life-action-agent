...
## 0. 확정 결정 (LEAN) — 2026-06-02, codex 최종 검토

여러 번의 검토 끝에 **LEAN 스코프**로 확정:

- **주력 (이번 프로토타입)**: 진단(졸업 가능 판단) + 영역별 학점 갭 + **결정론 리스크 등급** + **단일 추천 학기 로드맵** + **시각 대시보드 컨설팅 보고서**.
- **후순위 (DEFER)**: 다중 경로 비교(대체과목/계절학기/조기졸업/마이크로디그리 A·B·C 시나리오 비교). FULL은 시간·발표 리스크 → 나중에.
- **로드맵 엔진 = "LLM이 짜고 + 결정론 검증기"** (deterministic solver 아님). LLM이 *주어진 정형 사실*(후보 과목·개설학기·선수과목·갭)로 로드맵을 생성 → **결정론 validator가 하드 제약 재검사 → 1회 repair → 실패 시 "안 되는 이유" 솔직히 표시(가짜 계획 금지)**. 검증기+repair+honest-failure가 LLM을 자유생성이 아니라 *바운드*로 묶음 → 프로토타입 규모(한 학과·수십 과목)에 적정.
- **agentic 코어 프레이밍**: "LLM이 로드맵을 짠다"가 아니라 **"바운드 졸업사정 에이전트"** — 정형 기록을 검증하고, 정확한 학과 카탈로그와 대조하고, 리스크를 잡고, 실현가능한 학기 로드맵을 *repair*해, 근거 달린 컨설팅 대시보드를 산출.
- **데모에서 가장 중요한 것**: 엑셀 → 코드 카탈로그 매칭 → F/재수강 검증 → **정확한 학점 갭 진단**이 *의심할 여지 없이 믿음직*할 것. (정합성이 신뢰의 토대)
- **경로 비교는 빼되 "왜 이 계획인지"는 남긴다**: 추천 이유 / 안 되는 이유("2학기 내 불가 — 전공필수 2과목 같은 학기만 개설") / 제약 완화 시 대안 / 가정("계절학기 미사용·최대 N과목") / 과목별 요람 근거.

> 아래 §1~§11은 배경·근거·세부 설계. 충돌 시 **이 §0이 우선**.
...
2. **로드맵 = LLM이 짜고 결정론 validator가 검증**(§0). LLM에 *정형 사실*(후보 과목·개설학기·선수과목·갭·제약)을 데이터로 주고 로드맵 생성 → validator가 하드 제약(과목 실재·개설학기·선수순서·학점 충족) 재검사 → 1회 repair → 실패 시 honest-failure. 제약은 system 프롬프트, 사실은 입력 데이터. (다중 경로 *비교*는 후순위 — §0.)
...
LEAN 기준, JSON-first 데이터로 그리는 컨설팅 보고서(로드맵 중심):
1. **종합 판정**(맨 위, 결론 먼저): 리스크 등급 + 한 줄 결론 + 졸업 예상 시점
2. 컨텍스트 카드: 입학연도·본전공·다/부전공·적용 요람
3. **검증 테이블**: 원본 → 확정값, 편집 가능, 폐강/F/재수강 제외 토글
4. 영역별 학점 게이지: 총학점·전공필수·전공선택·기초교양·핵심교양·자유교양
5. 핵심 이슈(부족·위험): 부족 학점·누락 필수과목·근거 page
6. **추천 학기별 로드맵 타임라인 + "왜 이 계획인지"** (← 주인공)
7. 다음 단계 체크리스트
8. 근거(citation): `G1/G2` 생성·검증 유지, **기본 접힘 + "근거 보기" 토글**

> 다중 경로 비교 카드는 후순위(§0). 대신 로드맵에 **추천 이유 / 안 되는 이유 / 제약 완화 시 대안 / 가정**을 붙여 "왜 이 계획인지"를 설명.

보고서 mockup:
```
┌ 📋 졸업사정 컨설팅 리포트  ·  AI빅데이터융합경영학과 · 2024학번 · 본전공+데이터과학 부전공 ┐
│ 종합 판정: 🟡 주의  "정규 2학기로 졸업 가능, 단 전공선택 6학점 보완 필요"  졸업 예상 2027-2 │
│ ① 현황   총학점 ▓▓▓▓▓▓▓▓░░ 108/130 · 전공필수 18/18✅ · 전공선택 24/30 ⚠️6부족        │
│ ② 핵심이슈  • 전공선택 6학점 부족[G2]  • '데이터마이닝'(전필) 미이수 — 2학기만 개설[G3]   │
│ ③ 추천 로드맵  2026-2│데이터마이닝·머신러닝·교양   2027-1│캡스톤·전선·교양              │
│    왜 이 계획: 남은 2학기·계절학기 불가·관심분야(데이터분석) 부합[G5]                     │
│ ④ 다음 단계  ☐ 데이터마이닝 수강가능 학과 확인  ☐ 부전공 인정학점 확인                   │
│ ▸ 근거 보기 (요람 출처) [접힘]                                                          │
└──────────────────────────────────────────────────────────────────────────────────────┘
```

**값싼 데모 강화 요소**: assumptions 패널("이 데이터는 학생 검증 완료 기준"), 근거 토글, 리스크 라벨, **노드 트레이스**(`요람 로딩 → 데이터 검증 → 갭 계산 → 로드맵 플래닝 → 검증/repair → 리스크 산정` — Dify식 시각화).
